...
export const search = tool({
  ...
  async execute({ query, limit, agent, days, fields }, ctx) {
    ...
    try {
      ...
      // If parsing fails, try to parse lines with paths and sort by file stat mtime
      try {
        const lines = output.split('\n').filter(l => l.trim());
        const withMtime = lines.map(line => {
           // simple regex to find a file path at the start of the line or in a standard format
           const match = line.match(/(?:^|\s|\/)([a-zA-Z0-9_.\-/]+(?:[a-zA-Z0-9_.\-]+))/);
           let mtime = 0;
           if (match) {
             try {
               mtime = statSync(match[1]).mtimeMs;
             } catch {}
           }
           return { line, mtime };
        });
        // Sort if any lines actually got an mtime
        if (withMtime.some(item => item.mtime > 0)) {
           withMtime.sort((a, b) => b.mtime - a.mtime);
           return withMtime.map(item => item.line).join('\n');
        }
      } catch {}
